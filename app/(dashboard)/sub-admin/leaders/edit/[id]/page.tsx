...
import { useGetLeaderByIdQuery, useUpdateLeaderMutation } from "@/redux/api/adminApi";
import { toast } from "sonner";

export default function EditLeaderPage() {
  const router = useRouter();
  const { id } = useParams();
  const { data: leaderData, isLoading: isFetching } = useGetLeaderByIdQuery(id as string);
  const [updateLeader, { isLoading: isUpdating }] = useUpdateLeaderMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateLeader({ id: id as string, data }).unwrap();
      toast.success("Leader updated successfully");
      router.push("/sub-admin/leaders");
    } catch (error: any) {
      toast.error(error?.data?.message || "Failed to update leader");
    }
  };
  ...
  if (isFetching) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Loader2 className="h-8 w-8 animate-spin text-[#4F00FF]" />
      </div>
    );
  }
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col gap-1 mb-8">
              ...
              <p className="text-sm text-gray-500">
                Update leadership team profile
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-end gap-3">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => router.push("/sub-admin/leaders")}
